...
query_templates = [
    # Simple Selects
    (lambda table, column: (f"SELECT {column} FROM {table};", f"Selects the first 5 rows of the column '{column}' from the table '{table}'."), ['any/*']),
    ...
    (lambda table, columns: (f"SELECT {columns[0]}, {columns[1]} FROM {table};", f"Selects the first 5 rows of columns '{columns[0]}' and '{columns[1]}' from '{table}'."), ['categorical/other', 'any']),
    (lambda table, columns: (f"SELECT {columns[0]}, {columns[1]} FROM {table} ORDER BY {columns[1]} DESC;", f"Selects '{columns[0]}' and '{columns[1]}' ordered by '{columns[1]}' in descending order, limiting to 5 rows."), ['categorical/others', 'numeric']),
    (lambda table, columns, min_max: (f"SELECT {columns[0]}, {columns[1]} FROM {table} WHERE {columns[1]} > {random_number(*min_max)};", f"Selects '{columns[0]}' and '{columns[1]}' where '{columns[1]}' is greater than a random value, limited to 10 results."), ['categorical/others', 'numeric']),
    ...
    (lambda table, columns, min_max: (f"SELECT {columns[0]}, COUNT(*), SUM({columns[1]}) FROM {table} GROUP BY {columns[0]} HAVING {columns[1]} > {random_number(*min_max)};", f"Groups by '{columns[0]}', counts rows, and sums '{columns[1]}' where '{columns[1]}' is greater than a random value."), ['categorical/others', 'numeric']),
    ...
    (lambda table, column, min_max: (f"SELECT {column} FROM {table} WHERE {column} BETWEEN {random_number(*min_max)} AND {random_number(*min_max)};", f"Selects rows where '{column}' falls between two random values."), ['numeric']),
    ...
    (lambda table, columns, min_max: (f"SELECT {columns[0]}, SUM({columns[1]}) FROM {table} GROUP BY {columns[0]} HAVING SUM({columns[1]}) > {random_number(*min_max)};", f"Groups by '{columns[0]}', calculates the sum of '{columns[1]}', and returns groups where the sum exceeds a random value."), ['categorical', 'numeric']),
    ...
    # (lambda table, columns, min_max: (f"SELECT {columns[0]}, {columns[1]} FROM {table} WHERE {columns[1]} BETWEEN {random_number(*min_max)} AND {random_number(*min_max)};", f"Selects '{columns[0]}' and '{columns[1]}' where '{columns[1]}' falls between two random values."), ['any', 'numeric']),
    ...
    # (lambda table, columns: (f"SELECT {columns[0]}, {columns[1]} FROM {table} WHERE EXISTS (SELECT 1 FROM {table} WHERE {columns[1]} > 100 AND {columns[0]} = {table}.{columns[0]});", f"Selects '{columns[0]}' and '{columns[1]}' where there exists a row with '{columns[1]}' greater than 100 and matching '{columns[0]}' values."), ['any', 'numeric']),
